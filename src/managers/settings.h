...
private:
    ...
    Q_PROPERTY(int    term_type           READ get_term_type           WRITE set_term_type           NOTIFY changed_term_type          )

public:
    static Settings& Instance();

    int get_term_type(bool);

public Q_SLOTS:
    Q_INVOKABLE uint16_t get_wol_port           ();
    Q_INVOKABLE double   get_server_refresh_rate();
    Q_INVOKABLE QFont    get_font               ();
    Q_INVOKABLE int      get_term_type          ();
    ...
    Q_INVOKABLE void set_term_type          (int      value);
